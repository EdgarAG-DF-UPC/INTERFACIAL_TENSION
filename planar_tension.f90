...
! Data de creació: 10 d'abril del 2025
! Última modificació: 06 / 05 / 2025
...
PROGRAM ST
    ...
    TYPE peratom
        ...
        character*2 nom
        real*8 massa
        real*8, dimension(3) :: posicio, forca, velocitat
    ...
    REAL*8 INT_TRAP, INTERF_ENER, Rs, INTEGRAL, tensio
    REAL*8 distancia, distancia_1D, dist
    REAL*8 Ekin, Epot, Epi
    REAL*8 COM
    INTEGER N, NHe, NLi, NPb, NB
...
!   INDEXS:
    INTEGER i, j, k, l, m, p, q, s, t, ti, tj, n1, n2, n3, nt(3), dir
    INTEGER, dimension(:,:), allocatable :: PAR, PARL, BONDS
!   COMPTADORS (Àtoms totals, àtoms a la droplet, timesteps, etc.):
    INTEGER Nats, Nd, ts
    REAL*8 count
    INTEGER c1, c2, c0, ctot, c1i, c2i, c0i, ctoti
!   COORDENADES, FORCES, VELOCITATS, STRESS, etc.:
    REAL*8, dimension(3) :: posicio, velocitat, poscm, F_i, F_j
    REAL*8, dimension(:), allocatable :: edens, edensp
    REAL*8, dimension(:,:), allocatable :: pos, F, pos_LM, pos_He, force
    REAL*8, dimension(:,:), allocatable :: stress, PVM, PVMavg
!   QUANTITATS "PER ATOM":
...
!   MAGNITUDS RADIALS:
    ! REAL*8, dimension(:), allocatable :: r, gdr, z, r_ann, dens_r, pn, pk, pv, pvaver, pvp
!   MAGNITUDS EIX X:
    REAL*8, dimension(:), allocatable :: rx, dens_x, pvx, pvxaver, pkx, pnx
!   MAGNITUDS EIX Y:
    REAL*8, dimension(:), allocatable :: ry, dens_y, pvy, pvyaver, pky, pny
!   MAGNITUDS EIX Z:
    REAL*8, dimension(:), allocatable :: rz, dens_z, pvz, pvzaver, pkz, pnz
    REAL*8, dimension(:), allocatable :: pvp, num_he, num_li, num_pb, num
    REAL*8, dimension(:,:), allocatable :: virial
    ! REAL*8, dimension(:,:), allocatable :: dens, num, num_he, num_li, num_pb
    INTEGER, dimension(3) :: nr0, nr1, nr
    REAL*8 Dr
!   CLUSTERS:
...
!   COMMON BLOCKS:
    ...
    ! call Parallel_Hello_World(); stop
    ...
    allocate(particle(N))
    allocate(pos(N,3), F(N,3), Force(N,3))
    ...
    allocate(PAR(1:N*(N-1)/2, 2), PARL(1:(NLi+NPb)*(NLi+NPb-1)/2, 2), BONDS(1:N*(N-1)/2, 2))
    ...
    Dr = 0.1d0 !Angstroms
    nr(1) = nint(73.5d0 / Dr) ! 16.6d0 ~= L/2
    nr(2) = nint(73.5d0 / Dr)
    nr(3) = nint(140d0 / Dr)
    nr0(:) = -nr(:)/2
    nr1(:) = nr(:)/2
    ! allocate(r(nr0:nr1), gdr(nr0:nr1), z(nr0:nr1), pn(nr0:nr1), pk(nr0:nr1), pv(nr0:nr1), dens_r(nr0:nr1), num(3,nr0:nr1))
    allocate(dens_x(nr0(1):nr1(1)), dens_y(nr0(2):nr1(2)), dens_z(nr0(3):nr1(3)))
    allocate(rx(nr0(1):nr1(1)), ry(nr0(2):nr1(2)), rz(nr0(3):nr1(3)))
    do k = nr0(1), nr1(1)
        rx(k) = (dble(k) + 0.5d0)*Dr
    enddo
    do k = nr0(2), nr1(2)
        ry(k) = (dble(k) + 0.5d0)*Dr
    enddo
    do k = nr0(3), nr1(3)
        rz(k) = (dble(k) + 0.5d0)*Dr
    enddo
    allocate(num_he(nr0(3):nr1(3)),num_li(nr0(3):nr1(3)),num_pb(nr0(3):nr1(3)), num(nr0(3):nr1(3)))
    ! allocate(pnx(nr0(1):nr1(1)), pkx(nr0(1):nr1(1)), pvx(nr0(1):nr1(1)), pvxaver(nr0(1):nr1(1)))
    ! allocate(pny(nr0(2):nr1(2)), pky(nr0(2):nr1(2)), pvy(nr0(2):nr1(2)), pvyaver(nr0(2):nr1(2)))
    allocate(pnx(nr0(3):nr1(3)), pkx(nr0(3):nr1(3)), pvx(nr0(3):nr1(3)), pvxaver(nr0(3):nr1(3)))
    allocate(pny(nr0(3):nr1(3)), pky(nr0(3):nr1(3)), pvy(nr0(3):nr1(3)), pvyaver(nr0(3):nr1(3)))
    allocate(pnz(nr0(3):nr1(3)), pkz(nr0(3):nr1(3)), pvz(nr0(3):nr1(3)), pvzaver(nr0(3):nr1(3)), pvp(nr0(3):nr1(3)))
    allocate(virial(1:3, nr0(3):nr1(3)))
    allocate(stress(N, 3), PVM(nr0(3):nr1(3), 3), PVMavg(nr0(3):nr1(3), 3))
    do i = 1, N
        allocate(particle(i)%stress(nr0(3):nr1(3)))
    enddo
...
!   OBRIM ELS ARXIUS DE LECTURA
    ...
    do s = sni, snf
        ...
        do i = 1, N
            read(15,*) k, pos(i,:)
            read(19,*) particle(i)%velocitat
            do p = 1, 3
                if (pos(i,p) .lt. boxl(p)) then
                    print*, "WARNING!", pos(i,p), "<", boxl(p)
                    pos(i,p) = pos(i,p) + (boxh(p) - boxl(p)) 
                elseif (pos(i,p) .gt. boxh(p)) then
                    print*, "WARNING!", pos(i,p), ">", boxh(p)
                    pos(i,p) = pos(i,p) - (boxh(p) - boxl(p))
                endif
            enddo
            particle(i)%id = i
        ...
        do k = 1, N*(N-1)/2
            i = PAR(k,1)
            j = PAR(k,2)
            if (distancia(pos(i,:),pos(j,:),.True.) .lt. 9.01d0) then
                NB = NB + 1
                BONDS(NB,1) = i
                BONDS(NB,2) = j
            endif
        ...
        ! RTD%potener, RTD%kinener, RTD%enertail
...
! Identificació de clústers:
        ...
        allocate(coord(NHe), masscl(NHe))
        do p = 1,3
            do i = 1, NHe
                if ((He(i)%posicio(p) .lt. boxl(p)) .or. (He(i)%posicio(p) .gt. boxh(p))) then
                    print*, He(i)%posicio(p), boxl(p), boxh(p)
                    print*, He(i)%posicio
                    print*, He(i)%nom
                    print*, boxl
                    print*, boxh
                ...
                coord(i) = He(i)%posicio(p) - boxl(p)!coord(k) = pos_He(i,p) - boxl(p)
                ! print*, He(i)%posicio(p), "---->", coord(k)
                masscl(i) = massa_He
            enddo            
            poscm(p) = COM(coord, NHe, boxh(p)-boxl(p), masscl, .True.) + boxl(p) !particle(NLi+NPb+1:N)%massa
        enddo
        ! call PBC(poscm)
        do p = 1, 3
            if (poscm(p) .lt. boxl(p)) then
                print*, "WARNING!", poscm(p), "<", boxl(p)
                poscm(p) = poscm(p) + (boxh(p) - boxl(p)) 
            elseif (poscm(p) .gt. boxh(p)) then
                print*, "WARNING!", poscm(p), ">", boxh(p)
                poscm(p) = poscm(p) - (boxh(p) - boxl(p))
            endif
        enddo
        poscm(1) = 0d0
...
! CÀLCUL DEL TERME CINÈTIC: pk = rho(r) k T
        do i = 1, N
            p = 3
            ! do p = 1, 3
                k = floor(distancia_1D(p, poscm(:), pos(i,:), .True.)/Dr) + 1
                if ((k .ge. nr0(p)).and.(k .le. nr1(p))) then
                    num(k) = num(k) + 1d0
                    dens_z(k) = dens_z(k) + 1d0
            ...
            ! enddo
        enddo
        ! dens_r(:) = dens_r(:) / (4d0 * pi * Dr * (r(:)*r(:) + Dr*Dr/12d0))
        ! dens_x(:) = dens(1,:) / Dr      / (zh-zl) / (yh-yl)
        ! dens_y(:) = dens(2,:) / (xh-xl) / Dr      / (zh-zl)
        dens_z(:) = dens_z(:) / (xh-xl) / (yh-yl) / Dr
        pkz(:) = kB * RTD%temperatura * dens_z(:)
...
! CÀLCUL DEL TERME CONFIGURACIONAL: pv
        ...
        if (EAM) then
          ...
          !$OMP PARALLEL PRIVATE(psiij,dpsiij,i,j,ti,tj,edensp) SHARED(edens)
            edensp(:) = 0d0
          !$OMP DO
            do k = 1, NB!(NLi+NPb)*(NLi+NPb-1)/2
                i = BONDS(k,1) !PARL(k,1)
                j = BONDS(k,2) !PARL(k,2)
                ti = particle(i)%tipus!LM(i)%tipus
                tj = particle(j)%tipus!LM(j)%tipus
                if ((ti.ne.2).and.(tj.ne.2)) then
        ...
        Force = 0d0
        PVM = 0d0
        !!!!!!!!!!!!!!!!$OMP PARALLEL PRIVATE(Epi,pvp,virial,Fi,dFi,i,j,ctoti,c1i,c2i,c0i) SHARED(Epot,pvx,pvy,pvz,F,ctot,c1,c2,c0)
        !$OMP PARALLEL PRIVATE(Epi,pvp,virial,Fi,dFi,F_i,F_j,F,i,j,stress) SHARED(Epot,pvx,pvy,pvz,Force,PVM,m)
        if (EAM) then
            Epi = 0d0
            !$OMP DO
        ...
        !     do j = i+1, N
            ...
            if (distancia(pos(i,:),pos(j,:),.True.) .le. 9.01) then
                ! call P_CONF_PARALLEL(3, i, j, pvp, c0i, c1i, c2i, ctoti)
                F_i(:) = F(i,:)
                F_j(:) = F(j,:)
                call VIRIAL_IK_PARALLEL(i, j, virial, F_i, F_j, .True.)
                ! call VIRIAL_IK_PARALLEL(i, j, virial, F_i, F_j, .False.) ! compute (only) diagonal components of the virial
        ...
        !$OMP CRITICAL
            ...
            if (dir .eq. 1) then
                ! pvz(:) = pvz(:) + pvp(:) / (yh - yl) / (zh - zl) / Dr
                pvx(:) = pvx(:) + virial(1,:) / (yh - yl) / (zh - zl) / Dr
                pvy(:) = pvy(:) + virial(2,:) / (yh - yl) / (zh - zl) / Dr
                pvz(:) = pvz(:) + virial(3,:) / (yh - yl) / (zh - zl) / Dr
            else if (dir .eq. 2) then
                ! pvz(:) = pvz(:) + pvp(:) / (xh - xl) / (zh - zl) / Dr
                pvx(:) = pvx(:) + virial(1,:) / (xh - xl) / (zh - zl) / Dr
                pvy(:) = pvy(:) + virial(2,:) / (xh - xl) / (zh - zl) / Dr
                pvz(:) = pvz(:) + virial(3,:) / (xh - xl) / (zh - zl) / Dr
            else if (dir .eq. 3) then
                ! pvz(:) = pvz(:) + pvp(:) / (xh - xl) / (yh - yl) / Dr
                pvx(:) = pvx(:) + virial(1,:) / (xh - xl) / (yh - yl) / Dr
                pvy(:) = pvy(:) + virial(2,:) / (xh - xl) / (yh - yl) / Dr
                pvz(:) = pvz(:) + virial(3,:) / (xh - xl) / (yh - yl) / Dr
            else
                print*, "Variable 'dir' must be either 1 (x), 2 (y) or 3 (z)..."
            endif
            ! ctot = ctot + ctoti
            ! c0 = c0 + c0i
            ! c1 = c1 + c1i
        ...
        do k = 1, NB! N*(N-1)/2
            ...
            !     do j = i+1, N
                ...
                j = BONDS(k,2) !PAR(k,2)
                do p = 1, 3
                    ! particle(i)%stress(p) = particle(i)%stress(p) + (pos(i,p)-poscm(p))*force(i,p) + (pos(j,p)-poscm(p))*force(j,p)
                    STRESS(i,p) = STRESS(i,p) + distancia_1D(p,poscm(:),pos(i,:),.True.)*force(i,p) + &
                    distancia_1D(p,poscm(:),pos(j,:),.True.)*force(j,p)
                enddo
                do p = 1, 3
                    ! particle(i)%stress(p) = particle(i)%stress(p) + (pos(i,p)-poscm(p))*force(i,p) + (pos(j,p)-poscm(p))*force(j,p)
                    STRESS(j,p) = STRESS(j,p) + distancia_1D(p,poscm(:),pos(i,:),.True.)*force(i,p) + &
                    distancia_1D(p,poscm(:),pos(j,:),.True.)*force(j,p)
                enddo
        enddo
        !$OMP END DO
        ...
        do i = 1, N
            do p = 1, 3
                m = floor(distancia_1D(p, poscm(:), pos(i,:), .True.)/Dr) + 1
                if ((m.ge.nr0(3)).and.(m.le.nr1(3))) PVM(m,p) = PVM(m,p) + &
                (particle(i)%massa*1e-3 *particle(i)%velocitat(p)*particle(i)%velocitat(p) * 1e4/NA/e + &
                0.5d0*STRESS(i,p)) / (xh-xl) / (yh-yl) / Dr
            enddo
        enddo
        !$OMP END CRITICAL
        !$OMP BARRIER
        ...
        PVMavg = PVMavg + PVM
        ! do k = nr0(3), nr1(3)
        !     write(100+s,*) xh-xl, yh-yl, rz(k), pkz(k), pvx(k), pvy(k), pvz(k), PVM(k,:)
        ! enddo        
    enddo
    close(15)
...
! PROMITJOS:
    ...
    pvxaver = pvxaver / dble(snf-sni)
    pvyaver = pvyaver / dble(snf-sni)
    pvzaver = pvzaver / dble(snf-sni)
    PVMavg = PVMavg / dble(snf-sni)
    count = 0
    open(97,FILE="r_rho_pv.dat")
    ! do k = nr0, nr1
    !     dens_r(k) = num(k) / (4d0 * pi * Dr * (r(k)*r(k) + Dr*Dr/12d0))
    !     pvaver(k) = pvaver(k) / (4d0*pi*r(k)*r(k)*r(k))
    !     write(97,*) r(k), num_li(k), num_pb(k), num_he(k), kB*Temp*dens_r(k), pvaver(k)
    !     count = count + num(k)
    ! enddo
    ! do k = nr0(1), nr1(1)
    !     dens_x(k) = num(1,k) / Dr      / (zh-zl) / (yh-yl)
    !     write(97,*) rz(k), kB*Temp*dens_x(k)
    ! enddo
    ! do k = nr0(2), nr(2)
    !     dens_y(k) = num(1,k) / (xh-xl) / Dr      / (zh-zl)
    !     write(98,*) rz(k), kB*Temp*dens_y(k)
    ! enddo
    
    do k = nr0(3), nr1(3)
        dens_z(k) = num(k) / (xh-xl) / (yh-yl) / Dr     
        write(97,*) xh-xl, yh-yl, rz(k), kB*Temp*dens_z(k), pvxaver(k), pvyaver(k), pvzaver(k), PVMavg(k,:)
    enddo
    ...
    pnx(:) = kB*Temp*dens_z(:) + pvxaver(:)
    pny(:) = kB*Temp*dens_z(:) + pvyaver(:)
    pnz(:) = kB*Temp*dens_z(:) + pvzaver(:)
    ...
    print*, "---------------- CÀLCUL FINAL ----------------"
    print*, "----------------------------------------------"
    ! call CALCUL_TENSIO_PLANAR
    tensio = 0d0
    do k = nr0(3), nr1(3)
        tensio = tensio + pvzaver(k) - .5d0*pvxaver(k) - .5d0*pvyaver(k)
    enddo
    tensio = tensio * Dr
    print*, tensio, "eV/Å²", tensio * (1d20 * e), "N/m"
    print*, "----------------------------------------------"
    print*, "----------------------------------------------"
...
CONTAINS
    ...
    SUBROUTINE WRITE_POTS(SI)
        ...
        IF (SI) THEN
            ...
            enddo
        ELSE
            ! print*, "Segur que no vols guardar els potencials?"
            return
        ENDIF
    ...
    REAL*8 FUNCTION DERIVADA(parella, r)
        ...
        elseif (parella .eq. "HeHe") then
            ...
            elseif (EAMTTSS) then
                ...
                ! print*, derivada, (eps_HeHe/sig_HeHe)*dU_LJ(r/sig_HeHe), "!!!!!!!!!!!!!!!!!!"
            else
                print*, "PROBLEMA AMB EL POTENCIAL He-He..."
                stop
            endif
    ...
    SUBROUTINE VIRIAL_PARALLEL(i, j, sum_Fij_rij, F) !Calcul pressió...................
        ...
        if (EAM) then
            ...
            !     dudr = ((dFi + dFj) * dpsi + DERIVADA(parella,rij))
            ! elseif (parella.eq."LiPb") then
            !     call electro(rij, psi, dpsi, p1(3), p2) ! COMPTE!!!! El terme p1 és creuat!
            !     rhoi = edens(i)
            !     call embedding_Li(rhoi,Fi,dFi)
            !     call electro(rij, psi, dpsi, p1(1), p2)
            !     rhoj = edens(j)
    ...
    SUBROUTINE P_CONF_PARALLEL(dir, i, j, pv, c0, c1, c2, ctot)
        IMPLICIT NONE
        integer, intent(in) :: dir, i, j
        real*8, dimension(nr0(dir):nr1(dir)), intent(inout) :: pv
        integer, intent(inout) :: c0, c1, c2, ctot
        real*8, dimension(3) :: ri, rj, rtall, replica_i, replica_j
        real *8 rmin, rmax, lmin
        real*8 ri2, rj2, rij, rij2, dij(1:3), sij(1:3), rrij, dUij, SUMA_1D
        ...
        REAL*8, dimension(nr0(dir):nr1(dir)) :: pvpar
        REAL*8 za, zb
        ...
        sij(:) = ri(:) + rj(:)
        ! sij(k) = SUMA_1D(dir, ri, rj, .True.) ! COMPTE!!! rj, ri no commuten... (varia un signe) --> el primer dels 2 ha de ser l'origen de dij(:)
        ...
        pvpar = 0d0
        ! kmin = floor(min(ri(dir), rj(dir))) + 1
        ! kmin = max(kmin, nr0(dir))
        ! kmax = floor(max(ri(dir), rj(dir))) + 1
        ! kmax = min(kmax, nr1(dir))
        do k = kmin, kmax
            replica_i(:) = ri(:)
            replica_i(3) = replica_i(3) - rz(k)
            call PBC_1D(3, replica_i)
            replica_j(:) = rj(:)
            replica_j(3) = replica_j(3) - rz(k)
            call PBC_1D(3, replica_j)
            dij(:) = replica_j(:) - replica_i(:) !+ 2d0*rz(k)
            sij(:) = replica_i(:) + replica_j(:) !+ 2d0*rz(k)
            rij = distancia(replica_i,replica_j,.False.)
            if (rij .le. 9.01d0) then
                if ((replica_i(3).gt.(rz(k)-0.5d0*Dr)) .and. (replica_i(3).lt.(rz(k)+0.5d0*Dr))) then
                    za = replica_i(3)
                elseif (replica_i(3).lt.(rz(k)-0.5d0*Dr)) then
                    za = rz(k)-0.5d0*Dr
                elseif (replica_i(3).gt.(rz(k)+0.5d0*Dr)) then
                    za = rz(k)+0.5d0*Dr
                endif
                if ((replica_j(3).gt.(rz(k)-0.5d0*Dr)) .and. (replica_j(3).lt.(rz(k)+0.5d0*Dr))) then
                    zb = replica_j(3)
                elseif (replica_j(3).lt.(rz(k)-0.5d0*Dr)) then
                    zb = rz(k)-0.5d0*Dr
                elseif (replica_j(3).gt.(rz(k)+0.5d0*Dr)) then
                    zb = rz(k)+0.5d0*Dr
                endif
                if (za.eq.zb) cycle
                if (real(dabs(za-zb)).gt.real(Dr)) print*, "WARNING!  |za-zb| > Δz ...", za, zb, Dr
                if (force_field .eq. 0)  dudr = (epsilon(i,j)/sigma(i,j))*dU_LJ(rij/sigma(i,j))
                if (EAM) then
                    if (parella.eq."LiLi") then
                        call electro(rij, psi, dpsi, p1(1), p2)
                ...
                ! if (dir .eq. 1) then
                !     lambda = (2d0 * rx(k) - sij(1)) / (dij(1))
                !     rtall(1) = rx(k)
                !     rtall(2) = 0.5d0*(sij(2)) + 0.5d0*lambda*(dij(2))
                !     rtall(3) = 0.5d0*(sij(3)) + 0.5d0*lambda*(dij(3))
                ! elseif (dir .eq. 2) then
                !     lambda = (2d0 * ry(k) - sij(2)) / (dij(2))
                !     rtall(1) = 0.5d0*(sij(1)) + 0.5d0*lambda*(dij(1))
                !     rtall(2) = ry(k)
                !     rtall(3) = 0.5d0*(sij(3)) + 0.5d0*lambda*(dij(3))
                ! elseif (dir .eq. 3) then
                !     lambda = (2d0 * rz(k) - sij(3)) / (dij(3))
                !     rtall(1) = 0.5d0*(sij(1)) + 0.5d0*lambda*(dij(1))
                !     rtall(2) = 0.5d0*(sij(2)) + 0.5d0*lambda*(dij(2))
                !     rtall(3) = rz(k)
                ! endif
                ! rrij = dabs(dot_product(rtall, dij))
                ! if ((dij(dir).ne.0d0) .and. (dabs(lambda).lt.1d0)) then
                !     pvpar(k) = pvpar(k) - rrij * dudr / rij / dabs(dot_product(rtall,rtall)) ! / z(k)
                ! endif
                ! if (dij(dir) .ne. 0d0) then
                !     pvpar(k) = pvpar(k) - (dudr * (dij(dir)) / rij) * (dij(dir) / dij(3)) * (zb - za)
                ! elseif ((zb-za) .eq. 0d0) then
                !     pvpar(k) = pvpar(k) - (dudr * (dij(dir)) / rij) * (dij(dir))
                ! endif
                if (isnan(pvpar(k))) then
                    print*, pos(i,:), "=====>", ri, "=====>", replica_i
                    print*, pos(j,:), "=====>", rj, "=====>", replica_j
                    print*, dudr, dij(dir), rij, zb, za, (dudr * (dij(dir)) / rij) * (dij(dir) / dij(3)) * (zb - za)
                    ! print*, dij, lambda, - rrij * dudr / rij!, dabs(dot_product(rtall,rtall))
                    print*, "ERROR! pvpar is NaN"
                    stop
                endif
                ! print*, k, pvpar(k)
                ! print*, dudr, dij(dir), rij, zb, za, (dudr * (dij(dir)) / rij) * (dij(dir) / dij(3)) * (zb - za)
        ...
        enddo
        ! if (dir .eq. 1) then
        !     pv(:) = pv(:) + pvpar(:) / (yh - yl) / (zh - zl)
        ! else if (dir .eq. 2) then
        !     pv(:) = pv(:) + pvpar(:)  / (xh - xl) / (zh - zl)
        ! else if (dir .eq. 3) then
        !     pv(:) = pv(:) + pvpar(:)  / (xh - xl) / (yh - yl)
        ! else
        !     print*, "Variable 'dir' must be either 1 (x), 2 (y) or 3 (z)..."
        ! endif
        pv(:) = pvpar(:)
    ...
    SUBROUTINE VIRIAL_IK_PARALLEL(i, j, VIRIAL, force_i, force_j, SETPBC)
        IMPLICIT NONE
        INTEGER, intent(in) :: i, j
        INTEGER k, kmin, kmax
        INTEGER p
        REAL*8, dimension(1:3, nr0(dir):nr1(dir)), intent(inout) :: VIRIAL
        REAL*8, dimension(1:3) :: force_i, force_j
        CHARACTER*4 parella
        REAL*8 ri2, rj2, rij, rij2, za, zb
        REAL*8 dudr, rhoi, rhoj, psi, dpsi, dU_LJ
        REAL*8, dimension(1:3):: dij, ri, rj, sij, replica_i, replica_j
        ...
        force_i(:) = force_i(:) + dudr * dij(:) / rij
        force_j(:) = force_j(:) - dudr * dij(:) / rij

        ! kmin = nr0(3)
        ! kmax = nr1(3)
        !IRVING-KIRKWOOD: straight path between ri and rj...
        kmin = floor(ri(3)/Dr) !+ 1
        replica_j = rj
        replica_j(3) = replica_j(3) - ri(3)
        if (SETPBC) call PBC_1D(3, replica_j)
        replica_j(3) = replica_j(3) + ri(3)
        kmax = floor(replica_j(3)/Dr) !+ 1
        if (kmin.lt.nr0(3)) then
            kmin = nr0(3)
        elseif (kmin.gt.nr1(3)) then
            kmin = nr1(3)
        endif
        if (kmax.lt.nr0(3)) then
            kmax = nr0(3)
        elseif (kmax.gt.nr1(3)) then
            kmax = nr1(3)
        endif
        do k = kmin, kmax
            if (rij .le. 9.01d0) then
                if ((dabs(distancia_1D(3,ri,(/0d0,0d0,rz(k)/),SETPBC)).le.(0.5d0*Dr))) then
                    za = ri(3)
                else
                    replica_i(:) = ri(:)
                    replica_i(3) = replica_i(3) - rz(k)
                    if (SETPBC) call PBC_1D(3, replica_i)
                    if (replica_i(3).lt.(-0.5d0*Dr)) then
                        za = rz(k)-0.5d0*Dr
                    elseif (replica_i(3).gt.(0.5d0*Dr)) then
                        za = rz(k)+0.5d0*Dr
                    else
                        print*, "???????", replica_i, ri
                    endif
                endif
                if ((dabs(distancia_1D(3,rj,(/0d0,0d0,rz(k)/),SETPBC)).le.(0.5d0*Dr))) then
                    zb = rj(3)
                else
                    replica_j(:) = rj(:)
                    replica_j(3) = replica_j(3) - rz(k)
                    if (SETPBC) call PBC_1D(3, replica_j)
                    if (replica_j(3).lt.(-0.5d0*Dr)) then
                        zb = rz(k)-0.5d0*Dr
                    elseif (replica_j(3).gt.(0.5d0*Dr)) then
                        zb = rz(k)+0.5d0*Dr
                    else
                        print*, "???????", replica_j, rj
                    endif
                endif
                if (real(dabs(za-zb)).gt.real(Dr)) print*, "WARNING!  |za-zb| > Δz ...", za, zb, Dr, rz(k), ri(3), rj(3), &
                distancia_1D(3,ri,(/0d0,0d0,rz(k)/),SETPBC), distancia_1D(3,rj,(/0d0,0d0,rz(k)/),SETPBC)
                if (force_field .eq. 0)  dudr = (epsilon(i,j)/sigma(i,j))*dU_LJ(rij/sigma(i,j))
                do p = 1, 3
                    if (dij(3) .ne. 0d0) then
                        VIRIAL(p,k) = VIRIAL(p, k) - (dudr * (dij(p)) / rij) * dij(p) * dabs(zb - za) / dabs(dij(3)) ! (zb - za) / dij(3)!
                        if (((zb-za)/dabs(za-zb)) .ne. ((dij(3))/(dabs(dij(3)))).and.((za.ne.zb))) then
                            print*, "----------------------------------------"
                            print*, rz(k)
                            print*, za , zb, (zb-za), (zb-za)/dabs(za-zb)
                            print*, ri(3), rj(3), dij(3), dij(3)/dabs(dij(3))
                            print*, replica_i(3), replica_j(3)
                        endif
                    elseif (((zb-za) .eq. 0d0).and.&
                        ((dabs(distancia_1D(3,ri,(/0d0,0d0,rz(k)/),SETPBC)).le.(0.5d0*Dr))).and.&
                        ((dabs(distancia_1D(3,rj,(/0d0,0d0,rz(k)/),SETPBC)).le.(0.5d0*Dr)))) then
                        VIRIAL(p,k) = VIRIAL(p, k) - (dudr * (dij(p)) / rij) * dij(p)
                    else
                        print*, "CUIDAOOOOOOOOO"
                        print*, za, zb, rz(k)-Dr/2d0, rz(k), rz(k)+Dr/2d0
                        print*, replica_i
                        print*, replica_j
                        print*, ((distancia_1D(3,ri,(/0d0,0d0,rz(k)/),SETPBC))), ri(3)-rz(k)
                        print*, ((distancia_1D(3,rj,(/0d0,0d0,rz(k)/),SETPBC))), rj(3)-rz(k)
                        print*, dij
                        print*, zl, zh
                    endif
                enddo
        ...
        enddo
        return
    END SUBROUTINE VIRIAL_IK_PARALLEL
